...
export class UserService {
  ...
  register(id:number, username: string, password: string){
    this.http.post('http://ec2-3-90-146-246.compute-1amazonaws.com:8081/login/create-account', new User(id, username, password))
    .subscribe((response)=>{
  ...
  attemptLogIn(id:number, username: string, password: string) {
    ...
    this.http.post('http://ec2-3-90-146-246.compute-1amazonaws.com:8081/login', loggingInAsUser /**new User(username, password)**/)
